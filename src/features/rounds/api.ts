  ...
  const roundsWithStats = await Promise.all(
    (rounds || []).map(async (round: any) => {
      ...
      return {
        ...
        budgetCode1: round.budget_code_1,
        budgetCode2: round.budget_code_2,
...
export async function createRound(input: CreateRoundInput): Promise<Round> {
  ...
  const { data: round, error } = await supabase
    ...
    .insert({
      ...
      budget_code_1: input.budgetCode1,
      budget_code_2: input.budgetCode2,
...
export async function updateRound(input: UpdateRoundInput): Promise<Round> {
  ...
  if (input.budgetCode1 !== undefined) updateData.budget_code_1 = input.budgetCode1;
  if (input.budgetCode2 !== undefined) updateData.budget_code_2 = input.budgetCode2;
...
/**
 * Delete a round and all associated submissions and files
 */
export async function deleteRound(id: string): Promise<void> {
    ...
    submissions.forEach((submission) => {
      if (submission.file_url) {
        try {
          ...
          console.error('Failed to parse file URL:', submission.file_url, error);
        }
      }
    });
  ...
  // 3. Delete all submissions (this will cascade delete related records)
  ...
  // 4. Delete the round itself (this will cascade delete round_participants)
