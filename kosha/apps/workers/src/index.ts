...
import {
  handleGetLawsFull,
  handleGetLawsRules,
  handleExportRulesCSV,
  handleGetRulesetMeta,
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      // ========================================================================
      // Law ETL Endpoints (Read-only public access)
      // ========================================================================

      // Get all law articles from laws_full table
      if (path === '/api/laws-full' && request.method === 'GET') {
        const response = await handleGetLawsFull(request, env);
        const headers = new Headers(response.headers);
        Object.entries(corsHeaders).forEach(([key, value]) => {
          headers.set(key, value);
        });
      ...
      if (path === '/api/laws-rules' && request.method === 'GET') {
        const response = await handleGetLawsRules(request, env);
        const headers = new Headers(response.headers);
        Object.entries(corsHeaders).forEach(([key, value]) => {
          headers.set(key, value);
        });
      ...
      if (path === '/api/laws-rules.csv' && request.method === 'GET') {
        const response = await handleExportRulesCSV(request, env);
        const headers = new Headers(response.headers);
        Object.entries(corsHeaders).forEach(([key, value]) => {
          headers.set(key, value);
        });
      ...
      if (path === '/api/ruleset-meta' && request.method === 'GET') {
        const response = await handleGetRulesetMeta(request, env);
        const headers = new Headers(response.headers);
        Object.entries(corsHeaders).forEach(([key, value]) => {
          headers.set(key, value);
        });
      ...
      if (path === '/api/category-stats' && request.method === 'GET') {
        const response = await handleGetCategoryStats(request, env);
        const headers = new Headers(response.headers);
        Object.entries(corsHeaders).forEach(([key, value]) => {
          headers.set(key, value);
        });
   ...
   * Runs daily at 2:00 AM KST to sync law changes
   */
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    console.log('🔄 Starting scheduled law synchronization...');
    console.log(`⏰ Cron: ${event.cron}`);
    console.log(`📅 Scheduled time: ${new Date(event.scheduledTime).toISOString()}`);

    try {
      // NOTE: This is a placeholder for actual synchronization logic
      // In production, you would:
      // 1. Call National Law API to check for changes
      // 2. Compare with existing data in laws_full table
      // 3. Update changed articles
      // 4. Log sync results to a sync_log table

      // For now, just log the execution
      console.log('✅ Scheduled sync placeholder executed successfully');

      // Example: Get latest sync date from database
      const latestResult = await env.DB.prepare(
        'SELECT MAX(last_amended_date) as latest FROM laws_full'
      ).first<{ latest: string | null }>();

      const latestDate = latestResult?.latest || 'N/A';
      console.log(`📊 Latest amendment date in database: ${latestDate}`);

      // TODO: Implement actual sync logic here
      // - Fetch changes from National Law API
      // - Update laws_full table
